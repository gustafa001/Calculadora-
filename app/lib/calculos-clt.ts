...
// ─────────────────────────────────────────────
// UTILITÁRIO INTERNO — contagem de meses calendário
// ─────────────────────────────────────────────
...
export function calcularRescisao(params: {
  ...
  // Meses proporcionais pelo calendário real (Bug 2 corrigido)
  const mesesProporcional = mesesEntreAniversarios(ultimoAniversario, dataDemissao);

  // Cursor do último mês completo
  const inicioPeriodoDias = new Date(ultimoAniversario);
  inicioPeriodoDias.setMonth(inicioPeriodoDias.getMonth() + mesesProporcional);

  // Dias trabalhados no mês parcial (Bug 3 corrigido)
  const diasTrabalhados = Math.max(
    0,
    Math.min(
      30,
      Math.floor(
        (dataDemissao.getTime() - inicioPeriodoDias.getTime()) /
          (1000 * 60 * 60 * 24)
      )
    )
  );

  // Saldo de salário proporcional aos dias
  const saldoSalario = round2((salario / 30) * diasTrabalhados);

  // 13º proporcional (meses + dias)
  const mesesDecimo = mesesProporcional + (diasTrabalhados >= 15 ? 1 : 0);
  const decimoTerceiroProporcional = round2((salario / 12) * mesesDecimo);

  // Férias proporcionais
  const mesesFerias = mesesProporcional + (diasTrabalhados >= 15 ? 1 : 0);
  ...
  // FGTS do mês — incide sobre saldo de salário (Bug 1 corrigido)
  const fgtsMes = round2(saldoSalario * 0.08);

  // Multa FGTS 40% sobre saldo acumulado informado
  ...
  // Base de INSS/IRRF: saldo + 13º proporcional + férias proporcionais
  // + férias vencidas + aviso prévio + 13º sobre aviso
  // (⅓ de férias, multa FGTS e FGTS do mês são isentos) — Bug 4 corrigido
  const baseDesconto =
    saldoSalario +
    decimoTerceiroProporcional +
    feriasProporcional +
...
export function calcularFerias(
  ...
  const baseINSS = salarioFerias + terco;
...
export function calcularHoraExtra(
  ...
  switch (tipo) {
    ...
    case "noturna": adicionalPct = 0.5 + 0.2; break;
